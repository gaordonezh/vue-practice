...
const { findMessagesByChannelId, createMessage } = useMessagesStore();
const { getContactById, getPeople } = useContactStore();
const { getChannelTitle } = useChannelsStore();
...
const messagesView = computed(() =>
  findMessagesByChannelId(channelId.value).map((message) => {
    const author = getContactById(message.author);
...
const addMessage = () => {
  createMessage(channelId.value, message.value);
  message.value = "";
};
...
<template>
  <div class="messages">
    <header>
      <h2>{{ getChannelTitle(Number(route.params.id)) }}</h2>
